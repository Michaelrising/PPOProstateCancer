...
class CancerControl(gym.Env, ABC):
    def __init__(self, patient, ppo_mode='train',glv_mode='offline', t=0.):
        ...
        self.glv_mode = glv_mode
        self.t = t
        self.K, self.pars, self.init_states, self.weight, \
        self.base, self.m1, self.m2_ad, self.m2_ai, drug_decay, drug_length, self.ad_end_c, self.ai_end_c, self.sl, max_episodes_steps = patient
        ...
        self.ad_end_c = min(self.ad_end_c, 0.98) if self.ppo_mode == 'train' else 1
        self.ai_end_c = max(self.ai_end_c, 0.8) if self.ppo_mode == 'test' else 0.8
        ...
        self.metastasis_ai_deque = deque(maxlen=max_episodes_steps)
        self.metastasis_ad_deque = deque(maxlen=max_episodes_steps)
    ...
    def step(self, action):
        ...
        # reward
        reward = (self.max_episodes_steps - self.steps) / self.max_episodes_steps * (1 - x[0] / self.init_states[0]) + 1 * c2 if not done else 0
        ...
        drug_penalty = 0.6 * sum(self.base**self.drug_penalty_index * d * np.array([.6, .4])) + 0.4 * dosage.sum()
        reward += (self.steps + 1) * (1 - x[1]/self.K[1]) - drug_penalty
        self.steps += 1
...
#        metastasis = np.array([self.metastasis_ad_deque.count(1), self.metastasis_ai_deque.count(1)])/self.m2
    ...
    def _utilize(self, t, action):
        ...
        potential = self.weight @ np.array([-vt, -vai])
    ...
    def reset(self, init_date):
        # online true means we start the ppo after the first circle
        self.steps = 0
        self.t = init_date
        self.states = self.init_states.copy()
        ...
        normalized_states = np.log10(self.states + 1) / np.log10(self.normalized_coef)
        normalized_df = np.zeros(3)
        df = self.cancerode.forward(t = init_date, y = self.states.reshape(-1))
        for i, dx in enumerate(df):
            if dx > 1:
                normalized_df[i] = (np.log10(dx) + 1) / np.log10(self.normalized_coef[i])
            elif dx < -1:
