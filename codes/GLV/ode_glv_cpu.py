...
class ODEGlv_CPU(xitorch.EditableModule):
    def __init__(self, training_date, patientNo, **params):
        # self.r = params["r"]
        self.A = params["A"]
        self.K = params["K"]
        self.pars = params["pars"]
        self.data = training_date
    ...
    # def new_response(self, t, dose):
    #     # get the cell counts and the PSA level of each day
    #     # 28 days for one action
    #     ###################
    #     ts = np.linspace(start=t, stop=t + 28 - 1, num=28, dtype=np.int)
    #     dose_leu = dose[1]
    ...
    #     self.new_drug = drug * np.array([1 / 200, 1 / 7.5])
    ...
    def forward(self, t, y):
        ...
        p = y[-1]  # psa level
        dxdt = torch.multiply(
            r * x, (1 - (x @ A / self.K) ** phi - response @ Beta))  # -
        dpdt = betac * sum(x) * self.cell_size - gamma * p
        df = torch.cat((dxdt, dpdt.view(-1)))
        return df

    def save_pars(self, plist, save_path):
