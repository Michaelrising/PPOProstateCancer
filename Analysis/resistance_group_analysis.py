...
# resistance = clustering[clustering.cluster == 0]
# uniform = [11, 12, 13, 19, 20, 25, 32, 58, 78, 88, 105]
# adaptive = [4, 17, 24, 36, 51, 52, 54, 62, 71, 75, 79, 85, 94, 96, 101, 102, 104]
# resistance = list(set(uniform + adaptive +[83]) - set([11, 102]))
...
for i in resistance:
    ...
    print(patientNo)
    # if i in uniform:
    #     resistance_uniform.append(patientNo)
    # else:
    #     resistance_adaptive.append(patientNo)
...
for patient_i in df_ppo_drug.index:
    ...
    on_percentage = 1 - off_percentage
...
on_clinical = pd.DataFrame(on_clinical, index = patientLables)
...
for i in resistance:
    ...
    if len(off_interval) < len(on_interval):
        for _ in range(len(on_interval) - len(off_interval)):
            off_interval.append(1)
    else:
    ...
    f_drug = interpolate.interp1d(x = np.arange(1, len(drug_dose_change)+1)/(len(drug_dose_change)), y = drug_dose_change, kind='nearest',fill_value="extrapolate")
...
for l, patient in enumerate(resistance_all):
    ...
    if len(off_on_ratio) >= 2:
        f = interpolate.interp1d(x=np.arange(1, len(off_on_ratio)+1) / max((len(off_on_ratio) - 1), 1), y=off_on_ratio, kind='nearest', fill_value="extrapolate")
        pred_x = np.linspace(0, 1, 100)
        OFF_ON_R_PRED.append(f(pred_x))
        f_cpa = interpolate.interp1d(x=np.arange(1, len(cpa_dose_change)+1) / max((len(cpa_dose_change) - 1), 1), y=cpa_dose_change, kind='nearest', fill_value="extrapolate")
        CPA_R_PRED.append(f_cpa(pred_x))
        f_leu = interpolate.interp1d(x=np.arange(1, len(leu_dose_change)+1) / max((len(leu_dose_change) - 1), 1),
